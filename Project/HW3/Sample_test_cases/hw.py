...
# state = np.zeros((rows , columns),dtype = float)
...
def iterative(state,grid,rows,columns,pWalk,pRun,factor,rWalk,rRun,theta,answer):
  ...
  # storage = np.zeros((rows * columns,16),dtype = int)
  for i in range (rows):
  ...
  while (1):
    # count = count + 1
    difference = 0 
    rewardMatrix = np.array([rWalk,rWalk,rWalk,rWalk,rRun,rRun,rRun,rRun])
    probablityMatrix = np.array([pWalk,pWalkRst,pWalkRst,pWalk, pWalkRst,pWalkRst,pWalk, pWalkRst,pWalkRst,pWalk, pWalkRst,pWalkRst,pRun,pWalkRst,pWalkRst,pRun,pWalkRst,pWalkRst,pRun,pWalkRst,pWalkRst,pRun,pWalkRst,pWalkRst])
    for i in range (rows - 1, -1, -1):
      for j in range (columns):
        ...
        if (grid[i][j] == 0):
          ...
          stateMatrix = np.array([ [np.concatenate(([ state[storage1[k][0]][storage1[k][1]],state[storage1[k][4]][storage1[k][5]], state[storage1[k][6]][storage1[k][7]]],np.zeros(21)))],\
            [np.concatenate((np.zeros(3),[ state[storage1[k][2]][storage1[k][3]],state[storage1[k][4]][storage1[k][5]], state[storage1[k][6]][storage1[k][7]] ],np.zeros(18))) ],\
            [np.concatenate((np.zeros(6),[ state[storage1[k][4]][storage1[k][5]],state[storage1[k][0]][storage1[k][1]], state[storage1[k][2]][storage1[k][3]] ],np.zeros(15))) ],\
            [np.concatenate((np.zeros(9),[ state[storage1[k][6]][storage1[k][7]],state[storage1[k][0]][storage1[k][1]], state[storage1[k][2]][storage1[k][3]] ],np.zeros(12))) ],\
            [np.concatenate((np.zeros(12),[ state[storage1[k][8]][storage1[k][9]],state[storage1[k][12]][storage1[k][13]], state[storage1[k][14]][storage1[k][15]] ],np.zeros(9))) ],\
            [np.concatenate((np.zeros(15),[ state[storage1[k][10]][storage1[k][11]],state[storage1[k][12]][storage1[k][13]], state[storage1[k][14]][storage1[k][15]] ],np.zeros(6))) ],\
            [np.concatenate((np.zeros(18),[ state[storage1[k][12]][storage1[k][13]],state[storage1[k][8]][storage1[k][9]], state[storage1[k][10]][storage1[k][11]] ],np.zeros(3))) ],\
            [np.concatenate((np.zeros(21),[state[storage1[k][14]][storage1[k][15]],state[storage1[k][8]][storage1[k][9]], state[storage1[k][10]][storage1[k][11]] ] ))] ])
          
          utilityMatrix = factor * np.matmul(probablityMatrix , stateMatrix) + rewardMatrix
          state[i][j] = utilityMatrix.max()
